...
TEST(SunsetTesting, MoonPhase) {
  ...
  EXPECT_EQ(sun.moonPhase(1579097467), 20)
      << "Returned: " << sun.moonPhase() << ", epoch is " << 1579097467;
}
...
TEST(SunsetTesting, MoonphaseNoArgs) {
  ...
  EXPECT_EQ(moonphase1, moonphase2)
      << "Moonphase no args != moonphase(std::time(0))";
}
...
TEST(SunsetTesting, MoonphaseZeroEpoch) {
  ...
  EXPECT_EQ(sun.moonPhase(0), -6)
      << "Returned: " << sun.moonPhase() << ", epoch is " << 0;
}
} // namespace
